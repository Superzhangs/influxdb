...
func TestService(t *testing.T) {
	...
	t.Run("InitStack", func(t *testing.T) {
		safeCreateFn := func(ctx context.Context, orgID influxdb.ID, stack Stack) error {
			return nil
		}

		type createFn func(ctx context.Context, orgID influxdb.ID, stack Stack) error
		...
		t.Run("when store call is successful", func(t *testing.T) {
			...
			stack, err := svc.InitStack(context.Background(), 9000, 3333)
			require.NoError(t, err)
		...
		t.Run("handles unexpected error paths", func(t *testing.T) {
			tests := []struct {
			...
			}{
				{
					...
					store: func() *fakeStore {
						return newFakeStore(func(ctx context.Context, orgID influxdb.ID, stack Stack) error {
			...
			for _, tt := range tests {
				fn := func(t *testing.T) {
					...
					_, err := svc.InitStack(context.Background(), 9000, 3333)
					require.Error(t, err)
...
type fakeStore struct {
	createFn func(ctx context.Context, orgID influxdb.ID, stack Stack) error
}

func (s *fakeStore) CreateStack(ctx context.Context, orgID influxdb.ID, stack Stack) error {
	if s.createFn != nil {
		return s.createFn(ctx, orgID, stack)
	}
...
func (s *fakeStore) UpdateStack(ctx context.Context, orgID influxdb.ID, stack Stack) error {
	panic("not implemented")
}
